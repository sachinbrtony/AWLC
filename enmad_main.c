...
void oled_display(char *msg, char *val, uint8_t str_x, 
            uint8_t str_y, unit8_t oled_val1, uint8_t val_x,
            uint8_t val_y, unit8_t oled_val2)
{
    SSD1306_GotoXY(str_x,str_y);
    oled_puts(msg, oled_val1);
    SSD1306_GotoXY(val_x , val_y);
    oled_puts(val, oled_val2);
}
...
const char* get_alm_msg(float ip_volt)
{
    ...
    switch(dryflag)
    {
        ...
        case 4:
            ...
            break;
        default:
            alm_msg = "Working Good!"
    }

    if (TANKWAR == 1)
        alm_msg = "Wirecut / Sensor error!"
...
void Display_ADC()
{
    ...
    /* OLED display */
    oled_display("MODE: ", mode, 1, 1, 1, 6, 1, 1);
    oled_display("MOTOR:", motor, 1, 2, 1, 6, 2, 1);
    oled_display("AC IN:", acvolt, 16, 1, 1, 16, 2, 1);
    oled_display("TANK:", TWL, 1, 4, 1, 1, 5, 2);
    oled_display("SUMP:", SWL, 12, 4, 1, 10, 5, 2);
    oled_display("ALM:", get_alm_msg(ACvolt), 1, 7, 1, 6, 7, 1);
}

void print_manf_name()
{
    oled_display("ENMAD", "SOLUTIONS", 1, 1, 2, 1, 5, 2);
    LEDchaser();
    LEDchaser();
    __delay_ms(50);
}
